...
void compress(char *source_file_name, char *destination_file_name) {
    ...
    source_file = fopen(source_file_name, "rb"); // "rb = read binary"
    compressed_file = fopen(destination_file_name, "wb"); // "wb = write binary"
    ...
    printf("\n\tComprimindo do arquivo incorreto.\n");

    freq_count(source_file, hasht); // Conta a frequência de cada caractere e armazena na tabela de hash.

    for (i = 0; i < SIZE_ASCII; i++) { // Enfileira cada item com frequência maior que zero na fila de prioridade.
...
void compress(char *source_file_name, char *destination_file_name) {
    ...
    char *trash_size = (char*) malloc(sizeof(char));
